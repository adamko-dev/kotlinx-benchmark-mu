...
tasks.withType<Test>().configureEach {
  javaLauncher = serviceOf<JavaToolchainService>().launcherFor { languageVersion = JavaLanguageVersion.of(8) }
}
      ...
      temporaryDir.resolve(embeddedKotlinPlugin.name).apply {
        writeText(
          embeddedKotlinPlugin.readText()
            ...
            .replace("\n        warn(\n", "\n        info(\n")
            // Mark internal things as internal to prevent compiler warnings
            // about unused code, and to stop them leaking into build scripts.
            .replace("\n\nfun Logger.", "\n\nprivate fun Logger.")
            .replace(
