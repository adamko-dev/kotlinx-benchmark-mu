...
//        return samples
  ...
  private fun saveBenchmarkResults(
    ...
    val message = result.run {
      val sample = score.sampleToText(config.mode, config.outputTimeUnit)
      val error = (error / score * 100).formatSignificant(2)
      "  ~ $sample ±${error}%"
    }
  ...
  override fun run(
    ...
    for (benchmark in benchmarks) {
      ...
      runWithParameters(
        names = suite.parameters,
        parameters = runnerConfiguration.parameters,
        defaults = suite.defaultParameters,
      ) { parameters ->
