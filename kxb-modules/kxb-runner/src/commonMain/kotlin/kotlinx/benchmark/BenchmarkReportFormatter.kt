...
sealed class BenchmarkReportFormatter {
  ...
  @KotlinxBenchmarkRuntimeInternalApi
  companion object {
    fun create(format: RunnerConfiguration.ResultFormat): BenchmarkReportFormatter =
      when (format) {
...
private object JsonBenchmarkReportFormatter : BenchmarkReportFormatter() {
  ...
  private fun format(result: ReportBenchmarkResult): String = buildJsonObject {
...
//      result.config.advanced.entries.forEach { (k, v) ->
//        put(k, v)
//      }
//    }

    putJsonObject("primaryMetric") {
      put("score", result.score)
      put("scoreError", result.error)
      putJsonArray("scoreConfidence") {
        add(result.confidence.first)
        add(result.confidence.second)
      }
      putJsonObject("scorePercentiles") {
        result.percentiles.entries.forEach { (k, v) ->
          put(k.format(2), v)
        }
      }
      put("scoreUnit", unitText(result.config.mode, result.config.outputTimeUnit))
      putJsonArray("rawData") {
        result.values.forEach { value ->
          add(value)
        }
      }
      putJsonObject("secondaryMetrics") {
        // no secondary metrics yet yet
      }
    }
