...
import kotlinx.benchmark.RunnerConfiguration.NativeFork
import kotlinx.benchmark.internal.KotlinxBenchmarkRuntimeInternalApi
import kotlinx.benchmark.native.BenchmarkRun.Companion.encodeToJson
...
//      ?.let {
//        NativeFork.entries.firstOrNull { entity -> entity.name.equals(it, ignoreCase = true) }
...
//    get() = "true".equals(advanced["nativeGCAfterIteration"], ignoreCase = true)
  ...
  private fun outputBenchmarks(
    ...
    start()
    benchmarks.forEach { descriptor ->
      val suite = descriptor.suite
      val config = BenchmarkConfiguration(runnerConfiguration, suite)
      var parametersId = 0
      runWithParameters(
        suite.parameters,
        runnerConfiguration.parameters,
        suite.defaultParameters,
      ) { params ->
        val benchmarkRunConfig = BenchmarkRun(
          benchmarkName = descriptor.name,
          config = config,
          parameters = params,
        ).encodeToJson()
        val fileName = "${additionalArguments[0]}/${suite.name}_${descriptor.name}_$parametersId.json"
  ...
  private fun List<BenchmarkDescriptor<Any?>>.getBenchmark(name: String): BenchmarkDescriptor<Any?> =
    find { it.name == name }
  ...
  private fun <T> warmup(
    ...
    is BenchmarkDescriptorWithNoBlackholeParameter -> {
      ...
      error("Unexpected ${benchmark::class.simpleName}")
  }
